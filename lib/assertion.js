...
Assertion.prototype.assert = function (expr, msg, negateMsg, expected, _actual) {
  var msg = util.getMessage(this, msg, negateMsg)
    , actual = util.getActual(this, _actual)
    , ok = util.test(this, expr);
...
Assertion.prototype.property = function (name, val) {
  var value = util.getPathValue(name, this.obj)
  ...
  if (undefined !== val) {
    this.assert(
      ...
      , 'expected ' + this.inspect + ' to have a property ' + inspect(name) + ' of ' +
          inspect(val) + ', but got ' + inspect(value)
  ...
  try {
    ...
    if (constructor && 'function' === typeof constructor) {
      ...
      this.assert(
        ...
        , 'expected ' + this.inspect + ' to throw ' + name + ' but a ' + err.name + ' was thrown'
