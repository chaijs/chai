...
assert.isIterable = function (obj, msg) {
  if (obj == undefined || typeof obj[Symbol.iterator] !== 'function') {
    msg = msg
...
/**
 * ### .containsSubset(target, subset[, msg])
 *
 * Asserts that `target` object/array deeply contains all of the provided
 ...
 *
 * When `target` is an object, `containsSubset` asserts that all of `subset`'s
 * properties exist in `target` with the same values, at the same nested depth.
 ...
 *     );
 *
 * When `target` is an array, `containsSubset` asserts that for each element
 ...
 * in `target` is passed to the function, and the assertion passes if the
 * function returns a truthy value.
 *
 *     assert.containsSubset({age: 25}, {age: (val) => val > 18});
 *
 * Add `.not` to use `doesNotContainSubset`.
 *
 * @name containsSubset
 * @alias containSubset
 * @param {unknown} val
...
/**
 * ### .doesNotContainSubset(target, subset[, msg])
 *
 * Asserts that `target` object/array does NOT deeply contain all of the
 * provided `subset`'s properties/elements. This is the negation of
 * `containsSubset`.
 *
 *     assert.doesNotContainSubset({a: 1, b: 2}, {a: 3});
 *     assert.doesNotContainSubset([1, 2, 3], [4, 5]);
 *
