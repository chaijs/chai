...
/**
 * Enables the `isAsync` flag such that all other chains will await the
 * target object
 */
function resolvesChainingBehavior() {
  flag(this, 'isAsync', true);
}

/**
 * Asserts that the target object eventually resolves
 *
 * @param {...*} args
 */
async function resolves(...args) {
  ...
  // TODO (43081j): decide if this is even an API shape we want
  // we could just say that `resolves()` means "this promise resolves", and
  // take no parameters.
  if (args.length > 0 && resolved) {
    const [expected] = args;
    // TODO (43081j): do negations get lost here?
    if (negate) {
      new Assertion(value, flagMsg, ssfi, true).not.equal(expected);
    } else {
      new Assertion(value, flagMsg, ssfi, true).equal(expected);
    }
  } else {
    this.assert(
      resolved,
...
Assertion.addChainableMethod('resolves', resolves, resolvesChainingBehavior);
